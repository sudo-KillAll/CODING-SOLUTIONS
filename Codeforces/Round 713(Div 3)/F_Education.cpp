...
void solve(){
    ...
    for(auto &i:b)  cin>>i;
    b.pb(0);
    ll prev_val = 0,prev_day=0;
    ll ans = 1e18;
    for(ll i=0;i<n;i++){
        ll x = (c-prev_val+a[i]-1)/a[i];
        ans = min(prev_day+max(x,(ll)0),ans);
        x = max((ll)0,(b[i]-prev_val+a[i]-1)/a[i]);

        prev_day+=x+1;
        prev_val+=x*a[i]-b[i];  
    }
    // ll x = (c-prev_val+a[n-1]-1)/a[n-1];
    // ans = min(ans,prev_day+max(x,(ll)0));
    //print(dp);
    cout<<ans<<"\n";
