...
void solve(){
    ll n;cin>>n;
    vi dp_a(n+1,0),dp_b(n+1,0),dp_c(n+1,0);

    for(ll i=1;i<=n;i++){
        ll a,b,c;cin>>a>>b>>c;
        dp_a[i]=a+max(dp_b[i-1],dp_c[i-1]);
        dp_b[i]=b+max(dp_a[i-1],dp_c[i-1]);
        dp_c[i]=c+max(dp_b[i-1],dp_a[i-1]);
    }

    cout<<max(dp_a[n],max(dp_b[n],dp_c[n]));
