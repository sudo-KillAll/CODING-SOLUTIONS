...
ll getAns(vector<vi> &v,ll i,ll j,ll chr){
    if(i>=j){
        if(i==0){
            return v[chr][i]>=1?0:1;
        }
        return v[chr][i]-v[chr][i-1]>=1?0:1;
    }
    ll seg1 = j/2-i+1,seg2 = j-j/2;
    chr++;
    ll value1 = getAns(v,i,j/2,chr),value2 = getAns(v,j/2+1,j,chr);
